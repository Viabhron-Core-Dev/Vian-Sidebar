...
class PwaServer(port: Int, private val zipFilePath: String, private val filesDir: File) : NanoHTTPD("127.0.0.1", port) {
    ...
    override fun serve(session: IHTTPSession): Response {
        ...
        try {
            ...
            com.example.LogKeeper.writeLog("PwaServer", "IOException - Failed to serve $uri: ${e.message}\n${android.util.Log.getStackTraceString(e)}")
            return newFixedLengthResponse(Response.Status.INTERNAL_ERROR, NanoHTTPD.MIME_PLAINTEXT, "500 Internal Server Error")
        } catch (e: java.net.SocketException) {
            com.example.LogKeeper.writeLog("PwaServer", "SocketException - Failed to serve $uri: ${e.message}\n${android.util.Log.getStackTraceString(e)}")
            return newFixedLengthResponse(Response.Status.INTERNAL_ERROR, NanoHTTPD.MIME_PLAINTEXT, "500 Internal Server Error")
        } catch (e: OutOfMemoryError) {
            com.example.LogKeeper.writeLog("PwaServer", "OutOfMemoryError - Failed to serve $uri: ${e.message}\n${android.util.Log.getStackTraceString(e)}")
            return newFixedLengthResponse(Response.Status.INTERNAL_ERROR, NanoHTTPD.MIME_PLAINTEXT, "500 Internal Server Error")
        } catch (e: Exception) {
            com.example.LogKeeper.writeLog("PwaServer", "Exception - Failed to serve $uri: ${e.message}\n${android.util.Log.getStackTraceString(e)}")
            return newFixedLengthResponse(Response.Status.INTERNAL_ERROR, NanoHTTPD.MIME_PLAINTEXT, "500 Internal Server Error")
        }
    ...
    private fun serveStream(session: IHTTPSession, inputStream: InputStream, mimeType: String, totalLength: Long): Response {
        try {
            ...
            com.example.LogKeeper.writeLog("PwaServer", "IOException in serveStream: ${e.message}\n${android.util.Log.getStackTraceString(e)}")
            throw e
        } catch (e: java.net.SocketException) {
            com.example.LogKeeper.writeLog("PwaServer", "SocketException in serveStream: ${e.message}\n${android.util.Log.getStackTraceString(e)}")
            throw e
        } catch (e: OutOfMemoryError) {
            com.example.LogKeeper.writeLog("PwaServer", "OutOfMemoryError in serveStream: ${e.message}\n${android.util.Log.getStackTraceString(e)}")
            throw e
        } catch (e: Exception) {
            com.example.LogKeeper.writeLog("PwaServer", "Exception in serveStream: ${e.message}\n${android.util.Log.getStackTraceString(e)}")
            throw e
        }
    }
